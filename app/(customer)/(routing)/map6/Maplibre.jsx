...
import {
  getCafesByIds,
  getLatestSuccessfulVisit,
  saveCafesBatch,
  saveVisitLog,
} from "@/lib/map/tileDBNew";
    ...
    try {
      ...
      await Promise.all(
        tiles.map(async ({ x, y, z }) => {
          const key = `${z}_${x}_${y}`;
          const routeKey = `tile:${key}`;
          const url = `/api/map/tile/${z}/${x}/${y}`;

          const start = performance.now();

          const cachedVisit = await getLatestSuccessfulVisit(routeKey);

          if (cachedVisit && Array.isArray(cachedVisit.cafeIds)) {
            ...
            if (uniqueCafeIds.length > 0) {
              const cafes = await getCafesByIds(uniqueCafeIds);
              const cachedFeatures = cafes
                .map((cafe) => cafeToFeature(cafe))
                .filter(Boolean);
          ...
          try {
            ...
            const cafes = features
              .map((feature) => featureToCafe(feature))
              .filter(Boolean);
            const cafeIds = cafes.map((cafe) => cafe.id);

            allFeatures.push(...features);
            await saveCafesBatch(cafes);
            await saveVisitLog({
              routeType: "tile",
              routeKey,
              status: "success",
      ...
      const dedupedFeatures = dedupeFeaturesByCafeId(allFeatures);

      allFeaturesRef.current = dedupedFeatures;
      setAllFeaturesState(dedupedFeatures);
      rebuildClusters(map, dedupedFeatures);
    } finally {
...
function featureToCafe(feature) {
  const id = String(feature?.properties?.id ?? "").trim();
  if (!id) return null;

  const name = String(feature?.properties?.name ?? "").trim();
  const image = String(feature?.properties?.image ?? "").trim();
  const [lng, lat] = feature?.geometry?.coordinates ?? [];

  if (!Number.isFinite(lat) || !Number.isFinite(lng)) return null;

  return { id, name, image, lat, lng };
}

function cafeToFeature(cafe) {
...
function dedupeFeaturesByCafeId(features) {
  const byId = new Map();

  features.forEach((feature) => {
    const cafe = featureToCafe(feature);
    if (!cafe) return;
    byId.set(cafe.id, cafeToFeature(cafe));
  });
